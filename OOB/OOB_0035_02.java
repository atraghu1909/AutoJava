...
public class OOB_0035_02 extends testLibs.BaseTest_OOB {
	...
	public void step2() {
		if (step1result) {
			...
			try {
				...
				versionScreen.activityAction("AD.3YR.MULTI.RATE.2YR", "", "Update").click();
				versionScreen.windowAction(SWITCH_TO_DEFAULT);
				compositeScreen.switchToFrame("id", "Transactions");
				tabbedScreen.findTab("Rate (Fixed/Floating)", "").click();

				try {
					inputData("RATE.TIER.TYPE", "Level", false);
					if (readData("DESCRIPTION:1").equalsIgnoreCase("AD.3YR.MULTI.RATE.2YR")) {
						Reporter.log("User can successfully access the version to create new Tier Rate Interest: "
