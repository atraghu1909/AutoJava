...
public class PERF_IL_ADMIN extends testLibs.BaseTest_Perf {

	// Expected Loading Times
	private static int load360 = 50;
	...
	public void customer360(final String productGroup) {

		element = "Customer 360 View";
		stepDescription = "Measure " + element + " loading time (all tabs)";
		stepExpected = element + " loading time is under " + load360 + " seconds per tab";
		...
		tabs.add("Customer");
		tabs.add("Address");
		tabs.add("Relationship");
		tabs.add("Employment");
		tabs.add("Financials");
		tabs.add("Documents / Images");
		tabs.add("Messages");
		tabs.add("Channel Preferences");
		tabs.add("Membership");
		tabs.add("Garnishment");
		tabs.add("Online Messages");
		tabs.add("Products");
		tabs.add("Investments");
		tabs.add("Loans");
		tabs.add("External Products");
		tabs.add("Cash Management");
		tabs.add("Beneficiaries");
		tabs.add("Alerts");
		tabs.add("Product Catalog");
		tabs.add("Registered Product");
		tabs.add("Safe Deposit Box");
		tabs.add("Card Management");
		tabs.add("EBill");
		...
		if (loginResult) {
			...
			if (customer == null || customer.contains(ERROR)) {
				latestError = "Error while creating customer to test loading times";
				stepActual = latestError;
				softVerify.fail(stepActual);
				stepResult = StatusAs.FAILED;
			} else {
				Reporter.log("Loading Customer 360 View", debugMode);
				timestampStart = new Timestamp(System.currentTimeMillis());
				action = "Initial ";
				findCIF(customer, customerType, ROLEBASED_LENDING);
				...
				if (versionScreen.waitForLoading()) {
					...
					if (enforceLimits && loadingTime > load360) {
					...
					for (final String currentTab : tabs) {
						...
						if (switchToTab(currentTab, "")) {
							...
							actualResult.append(action + element + " loading time was " + loadingTime + " seconds"
									...
									+ System.lineSeparator() + "End: " + timestampEnd.toString()
	...
	public void loanCreation(final String productGroup, final String product) {
		...
		if (customer == null || customer.contains("Error")) {
			...
			result = validateLimit(productGroup, product, customer, ROLEBASED_LENDING, "CUSTOMER:1,CURRENCY,",
	...
	public void paymentOrder(final String productGroup, final String product) {
		...
		if (mainArrangement == null || mainArrangement.contains("Error")) {
			...
			adHocID = createAdHocPayment("Open", "EFT", mainArrangement, paymentOrderFields, paymentOrderValues);

			timestampStart = new Timestamp(System.currentTimeMillis());
			action = "Validate ";
			toolElements.toolsButton(VALIDATE_DEAL).click();
			if (!adHocID.contains(ERROR) && versionScreen.waitForLoading()) {
				timestampEnd = new Timestamp(System.currentTimeMillis());
