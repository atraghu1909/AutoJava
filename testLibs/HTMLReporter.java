...
public class HTMLReporter extends AbstractReporter {
	...
	public void generateReport(final List<XmlSuite> xmlSuites, final List<ISuite> suites,
		...
		final boolean useFrames = System.getProperty(FRAMES_PROPERTY, "true").equals("true");
		final boolean onlyFailures = System.getProperty(ONLY_FAILURES_PROPERTY, "false").equals("true");
		final File outputDirectory = new File(outputDirectoryName, REPORT_DIRECTORY);
	 ...
	 */
	private void createFrameset(final File outputDirectory) throws Exception {
	...
	private void createOverview(final List<ISuite> suites, final File outputDirectory, final boolean isIndex,
			final boolean onlyFailures) throws Exception {
		final VelocityContext context = createContext();
	 ...
	 *            The target directory for the generated file(s).
	 */
	private void createSuiteList(final List<ISuite> suites, final File outputDirectory, final boolean onlyFailures)
			throws Exception {
		final VelocityContext context = createContext();
		context.put(SUITES_KEY, suites);
		context.put(ONLY_FAILURES_KEY, onlyFailures);
	 ...
	 *            The target directory for the generated file(s).
	 */
	private void createResults(final List<ISuite> suites, final File outputDirectory, final boolean onlyShowFailures)
			throws Exception {
		int index = 1;
		for (final ISuite suite : suites) {
			int index2 = 1;
			for (final ISuiteResult result : suite.getResults().values()) {
				final boolean failuresExist = result.getTestContext().getFailedTests().size() > 0
				...
				if (!onlyShowFailures || failuresExist) {
					...
					final String fileName = String.format("suite%d_test%d_%s", index, index2, RESULTS_FILE);
	 ...
	 */
	private SortedMap<IClass, List<ITestResult>> sortByTestClass(final IResultMap results) {
		final SortedMap<IClass, List<ITestResult>> sortedResults = new TreeMap<IClass, List<ITestResult>>(
				CLASS_COMPARATOR);
	 ...
	 *            The target directory for the generated file(s).
	 */
	private void createGroups(final List<ISuite> suites, final File outputDirectory) throws Exception {
		int index = 1;
		for (final ISuite suite : suites) {
			final SortedMap<String, SortedSet<ITestNGMethod>> groups = sortGroups(suite.getMethodsByGroups());
			if (!groups.isEmpty()) {
				final VelocityContext context = createContext();
				context.put(SUITE_KEY, suite);
				context.put(GROUPS_KEY, groups);
				final String fileName = String.format("suite%d_%s", index, GROUPS_FILE);
	 ...
	 *            The target directory for the generated file(s).
	 */
	private void createLog(final File outputDirectory, final boolean onlyFailures) throws Exception {
	...
	private SortedMap<String, SortedSet<ITestNGMethod>> sortGroups(
			final Map<String, Collection<ITestNGMethod>> groups) {
		final SortedMap<String, SortedSet<ITestNGMethod>> sortedGroups = new TreeMap<String, SortedSet<ITestNGMethod>>();
		for (final Map.Entry<String, Collection<ITestNGMethod>> entry : groups.entrySet()) {
			final SortedSet<ITestNGMethod> methods = new TreeSet<ITestNGMethod>(METHOD_COMPARATOR);
	 ...
	 */
	private void copyResources(final File outputDirectory) throws IOException {
		...
		final File customStylesheet = META.getStylesheetPath();
